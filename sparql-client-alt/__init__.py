...
The following packages are used:
  ...
  - for XML, the U{PyXML<http://pyxml.sourceforge.net/>}: C{http://pyxml.sourceforge.net/}
...
If C{SPARQLWrapper("http://localhost:2020/sparql",returnFormat=SPARQL.JSON)} was used, the result would be in
...
To make processing somewhat easier, the package can do some conversions automatically from the return result. These are:
  ...
  - for XML, the U{PyXML<http://pyxml.sourceforge.net/>} (C{http://pyxml.sourceforge.net/}) is
...
return formats should be different: eg, C{SPARQL.XML} means C{RDF/XML} and most of the SPARQL engines can also return the results
in C{Turtle}. The package, though it does not contain a full SPARQL parser,
makes an attempt to determine the query type when the query is set. This should work in most of the cases (but there is a possibility
to set this manually, in case something goes wrong).
...
from Wrapper      import SPARQLWrapper, XML, JSON, TURTLE, N3, GET, POST, SELECT, CONSTRUCT, ASK, DESCRIBE
