...
import SPARQL
from SPARQL.Wrapper import JSON, SELECT
import urllib2
from types import *
...
class Bindings :
	...
	def convert(self) :
		...
		Although C{Binding} is not a subclass of L{QueryResult<SPARQL.Wrapper.QueryResult>}, it is returned as a result by
		L{SPARQLWrapper2.query}, just like L{QueryResult<SPARQL.Wrapper.QueryResult>} is returned by
		L{SPARQL.SPARQLWrapper.query}. Consequently,
		having an empty C{convert} method to imitate L{QueryResult's convert method<SPARQL.Wrapper.QueryResult.convert>} may avoid unnecessary problems.
		"""
		return self
...
class SPARQLWrapper2(SPARQL.SPARQLWrapper) :
	"""Subclass of L{Wrapper<SPARQL.SPARQLWrapper>} that works with a JSON SELECT return result only. The query result is automatically set
	to a L{Bindings} instance. Makes the average query processing a bit simpler..."""
	def __init__(self,baseURI,defaultGraph=None) :
		...
		"""
		SPARQL.SPARQLWrapper.__init__(self,baseURI,returnFormat=JSON,defaultGraph=defaultGraph)

	def setReturnFormat(self,format) :
		"""Set the return format (overriding the L{inherited method<SPARQL.SPARQLWrapper.setReturnFormat>}).
	...
	def query(self) :
		...
		"""
		res = SPARQL.SPARQLWrapper.query(self)
	...
	def queryAndConvert(self) :
		...
		if self.queryType == SELECT :
			...
			return SPARQL.SPARQLWrapper.queryAndConvert(self)
