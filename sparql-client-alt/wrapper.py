...
# This is very ugly. The fact is that the key for the choice of the output format is not defined. Virtuoso uses 'format', joseki uses 'output',
# rasqual seems to use "results"
# Lee Feigenbaum told me that virtuoso also understand 'output' these days, so I removed 'format'. I do not have info about the
# others yet, ie, for the time being I keep the general mechanism. Hopefully, in a future release, I can get rid of that.
...
class SPARQLWrapper :
  """
	...
	def __init__(self,baseURI,returnFormat=XML,defaultGraph=None) :
		...
		if defaultGraph : self._querytext.append(("default-graph-uri",defaultGraph))
	...
	def resetQuery(self) :
		...
		self._querytext = []
		if self._defaultGraph : self._querytext.append(("default-graph-uri",self._defaultGraph))
	...
	def setReturnFormat(self,format) :
		...
		@param format: Possible values: are L{JSON}, L{XML}, L{TURTLE}, L{N3} (constants in this module). All other cases are ignored.
		@type format: string
		"""
	...
	def addExtraURITag(self,key,value) :
		"""
			...
			E.g., in virtuoso, one would add C{should-sponge=soft} to the query forcing virtuoso
	...
	def setQuery(self,query) :
		"""
			Set the SPARQL query text. Note: no check is done on the validity of the query (syntax or otherwise) by this module,
			except for testing the query type (SELECT, ASK, etc). Syntax and validity checking is done by the SPARQL service itself.
	...
	def _getURI(self) :
		...
		if self.returnFormat != XML :
			...
			for f in _returnFormatSetting: finalQueryText.append((f,self.returnFormat))
...
class QueryResult :
	...
	def _convertJSON(self) :
		...
		"""
		import simplejson
		return simplejson.load(self.response)
